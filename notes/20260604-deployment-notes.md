...
Setup steps are in `website/README.md`. The interesting part is *why* it
works without a browser warning:

A regular self-signed cert is signed by itself — no chain back to any CA
the browser knows, so it warns. mkcert instead creates a local CA and
installs it into the system trust store (macOS Keychain, Windows Certificate
Store, Firefox NSS). The browser sees a valid chain to a trusted CA and
stays silent — same mechanism as a real certificate, just locally scoped.

The local CA only exists on machines where you've run `mkcert -install`,
so it can't be used to impersonate anything outside your machine. For
production you still need a real cert (Let's Encrypt, etc.).
